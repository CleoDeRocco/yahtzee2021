...
General Program:
I believe the program satisfies all the requirements.
Some notes: I allowed users to enter zero for categories, but if they enter zero for yahtzee, yatzee bonus can't be had.
Additionally, I allowed users to play without entering a name, but to save their game they have to enter a name.
Values taht haven't been entered aren't deleted if the user doesn't delete them. Similarily, when a game change occurs (load or new game), the
values from the previous game are still in the text boxes (not in the score tallies), but boxes aren't disabled, so user can delete them.
Thanks for giving me the extra time to figure out local storage. Now I know how to disable my chrome settings to let me use it, so that is good!

ADDITIONS/EXTRA CREDIT:
Yahtzee bonus.
